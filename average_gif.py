...
def main():
    ...
    print(num_images) # 827
    print(len(all_points[0])) #68
    print("len(all_points): ", len(all_points)) # 823
    ...
    cv2.imwrite( "average_face_leader.jpg", 255 * output)
...
def gif_make_main():
    ...
    for i in range(0, num_images):
        ...
        # Apply similarity transformation
        img_affine = cv2.warpAffine(images[i], tform, (w, h))
        ...
        for i in range(0, len(images_norm)):
            ...
            for j in range(0, len(tri)):
                ...
                img = warp_triangle(images_norm[i], img, t_in, t_out)
    ...
    #print("raw[0].shape",raw[0].shape) #raw사진은 정상확인완료. Shape: (240, 170, 3)
        ...
        if file_path.endswith('.txt'):
            ...
            for i in list(range(len(points))):
                if i < 61 :
                    ...
                    #print(i,"elif문")
                    list_check_min = min([points[61][1],points[62][1],points[63][1]])
                    result_points.append((points[i][0], list_check_min))
                else:
                    #print(i,"else문")
                    list_check_max = max([points[65][1],points[66][1],points[67][1]])
                    result_points.append((points[i][0], list_check_max))
        
            points_array.append(result_points)

    return points_array


def read_points_old(path):
    # Create an array of array of points.
    points_array = []
...
def read_images(path):
    ...
    for file_path in sorted(os.listdir(path)):
        if file_path.endswith('.JPG'):
            # Read image found.RGB순서가 달라지므로 GIF 만들기 위해 코드 수정
            img = cv2.imread(os.path.join(path, file_path))[..., ::-1]
